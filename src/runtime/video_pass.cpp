...
void VideoUploads::Retain(const graph::ExecutionPlan& plan) {
...
void VideoUploads::Prepare(const graph::ExecutionPlan& plan, std::span<const VideoInput> inputs,
    ...
    for (const auto& instruction : plan.instructions_)
        if (instruction.operation_ == graph::Operation::kTextureVideo)
            PrepareNode(instruction, inputs, renderer);
}
void VideoUploads::PrepareNode(const graph::Instruction& instruction,
                               std::span<const VideoInput> inputs, render::Renderer& renderer) {
    if (inputs.size() > 4) throw std::length_error("video.instance_budget");
    if (instruction.operation_ != graph::Operation::kTextureVideo)
        throw std::invalid_argument("video.upload_instruction");
    ...
    if (!assets::ValidId(source) || !input->revision_ || !info.width_ || !info.height_ ||
        ...
        !std::isfinite(info.pixel_aspect_) || info.pixel_aspect_ <= 0 || info.pixel_aspect_ > 100 ||
        !std::isfinite(info.clockwise_rotation_) || !std::isfinite(input->gain_) ||
        input->gain_ < 0 || input->gain_ > 1)
