...
render::TextureHandle GlowPass::Draw(render::TextureHandle source, render::Extent extent,
    ...
    if (!extent.width_ || !extent.height_ || !finite(settings.strength_) ||
        ...
        settings.threshold_scale_ > 16 || settings.bloom_floor_ < 0 ||
        settings.bloom_floor_ > 1 || settings.luminance_cap_ < 0.01f ||
        settings.luminance_cap_ > 65504 || settings.levels_ < 1 || settings.levels_ > 6)
    ...
    if (extent != extent_ || levels != levels_ || !renderer.IsValid(output_.Handle())) {
        ...
        for (std::size_t index = 0; index + 1 < levels; ++index) {
            upsample_.push_back(renderer.CreateTexture(sizes[index], {},
                                                       render::TexturePrecision::kFloat16));
        }
        output_ = renderer.CreateTexture(extent, {}, render::TexturePrecision::kFloat16);
        extent_ = extent;
        levels_ = levels;
    }
