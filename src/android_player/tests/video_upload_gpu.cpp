...
void VerifyVideoUploadPixels(render::Renderer& renderer) {
    ...
    for (int sample = 0; sample < 3; ++sample) {
        ...
        frame->rgba_ = {static_cast<std::uint8_t>(sample * 60), 160, 80, 128};
        frame->seconds_ = sample;
        runtime::FrameContext context{double(sample), 1, {16, 16}, true};
        context.videos_ = {{1, id, frame, static_cast<std::uint64_t>(sample + 1), 1}};
        ...
        for (std::size_t index = 0; index < pixels.size(); index += 4)
            if (std::abs(int(pixels[index]) - sample * 30) > 2 ||
                std::abs(int(pixels[index + 1]) - 80) > 2 ||
                std::abs(int(pixels[index + 2]) - 40) > 2)
    ...
    std::cout << "Video upload: GLES frame replacement, alpha and stable allocation passed\n";
}
}  // namespace rhythm::validation
