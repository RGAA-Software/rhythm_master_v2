...
void Run(const rhythm::shader_authoring::Toolchain& tools, const std::filesystem::path& output) {
    ...
    const auto decoded = image_shader::Decode(std::span<const std::uint8_t>(
            reinterpret_cast<const std::uint8_t*>(bytes.data()), bytes.size()));
    Require(decoded.expression_ == request.expression_,
            "source travels with both target artifacts");
    ...
    for (const auto expression : {"while(true) {}", "vec4(uv, vec4(1.0))"}) {
    ...
    request.expression_ = "vec4(uv.x, uv.y, sin(time) * 0.5 + 0.5, 1.0)";
