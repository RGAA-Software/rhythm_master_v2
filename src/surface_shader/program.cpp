...
void Validate(const Program& program) {
    if (shader_expression::Validate(program.expression_, shader_expression::Profile::kSurfaceRgb) ||
        program.compiler_sha256_.size() != 64 ||
    ...
    shader_artifact::Validate(program.artifacts_[0], shader_artifact::Target::kWindowsSm5,
                              shader_artifact::Profile::kSurfaceRgb);
    shader_artifact::Validate(program.artifacts_[1], shader_artifact::Target::kGles300,
                              shader_artifact::Profile::kSurfaceRgb);
}
std::vector<std::uint8_t> Encode(const Program& program) {
    Validate(program);
    std::vector<std::uint8_t> bytes{'R', 'M', 'S', 'F', '0', '0', '1', '\n'};
...
Program Decode(std::span<const std::uint8_t> bytes) {
    constexpr std::array<std::uint8_t, 8> kMagic{'R', 'M', 'S', 'F', '0', '0', '1', '\n'};
