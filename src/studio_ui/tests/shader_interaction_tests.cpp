    ...
    try {
        ...
        for (const bool surface : {false, true}) {
            ...
            initial.document_.nodes_ = {
                    registry.MakeNode(1, surface ? "material.shader" : "texture.shader")};
            editor::History history(initial);
            studio::ShaderPanel panel;
            panel.SetTools(
                    {argv[1], argv[2], argv[3], std::filesystem::path(argv[3]).parent_path()});
            const auto assets =
                    std::filesystem::path(argv[4]) / (surface ? "surface" : "image") / "assets";
            ImGuiID source_id = 0;
            ...
            const char* replacement_source =
                    surface ? "vec3(uv.x, uv.y, sin(time))" : "vec4(uv.x, uv.y, sin(time), 1.0)";
            io.AddInputCharactersUTF8(replacement_source);
            frame();
            frame("###shader.compile");
            frame();
            drain();
            ...
            Check(std::get<assets::AssetId>(history.Current().document_.nodes_[0].properties_.at(
                          "asset")) == asset.id_,
                  "undo restores the previous successful shader");
            frame();
            drain();
            check_source(surface ? "0.5 + 0.5 * cos(time + uv.xyx * 6.283185 + vec3(0.0, 2.0, 4.0))"
                                 : "vec4(0.5 + 0.5 * cos(time + uv.xyx * 6.283185 + vec3(0.0, 2.0, "
                                   "4.0)), 1.0)");
            ...
            frame();
            drain();
            check_source(replacement_source);
            frame("###shader.compile");
            frame();
            drain();
