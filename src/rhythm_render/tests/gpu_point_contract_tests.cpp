...
void Run() {
    ...
    auto depth_small = renderer.CreateDepthTexture({8, 8});
    auto depth_map = renderer.CreateDepthTexture({16, 16});
    GpuPointStyle soft;
    soft.soft_depth_ = GpuPointSoftDepth{depth_map.Handle(), 0, 1, true, 0.1f};
    Reject([&] { renderer.SubmitGpuPoints(target.Handle(), stale, soft); });
    soft.soft_depth_ = GpuPointSoftDepth{depth_map.Handle(), 1, 11, true, 0};
    Reject([&] { renderer.SubmitGpuPoints(target.Handle(), stale, soft); });
    soft.soft_depth_ = GpuPointSoftDepth{atlas_map.Handle(), 1, 11, true, 0.1f};
    Reject([&] { renderer.SubmitGpuPoints(target.Handle(), stale, soft); });
    soft.soft_depth_ = GpuPointSoftDepth{depth_small.Handle(), 1, 11, true, 0.1f};
    Reject([&] { renderer.SubmitGpuPoints(target.Handle(), stale, soft); });
    soft.soft_depth_ = GpuPointSoftDepth{depth_map.Handle(), 1, 11, true, 0.1f};
    renderer.SubmitGpuPoints(target.Handle(), stale, soft);
