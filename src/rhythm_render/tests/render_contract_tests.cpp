...
int main() {
    ...
    try {
        ...
        }
        {
            ...
            draw.commands_[0].texture_ = color.Handle();
            draw.commands_[0].depth_linearization_.reset();
            draw.commands_[0].depth_of_field_ = DepthOfField{depth.Handle()};
            const auto circle_passes = renderer.Stats().passes_;
            renderer.Submit(output.Handle(), draw);
            Check(renderer.Stats().passes_ == circle_passes + 3);
            draw.commands_[0].depth_of_field_->shape_ = DepthOfFieldShape::kBox;
            draw.commands_[0].depth_of_field_->quality_ = DepthOfFieldQuality::kVeryLow;
            const auto half_box_passes = renderer.Stats().passes_;
            renderer.Submit(output.Handle(), draw);
            Check(renderer.Stats().passes_ == half_box_passes + 4);
            draw.commands_[0].depth_of_field_->quality_ = DepthOfFieldQuality::kMedium;
            const auto full_box_passes = renderer.Stats().passes_;
            renderer.Submit(output.Handle(), draw);
            Check(renderer.Stats().passes_ == full_box_passes + 3);
            draw.commands_[0].depth_of_field_->shape_ = static_cast<DepthOfFieldShape>(255);
            Reject([&] { renderer.Submit(output.Handle(), draw); });
            draw.commands_[0].depth_of_field_->shape_ = DepthOfFieldShape::kHexagon;
            draw.commands_[0].depth_of_field_->quality_ = static_cast<DepthOfFieldQuality>(255);
            Reject([&] { renderer.Submit(output.Handle(), draw); });
