...
SurfaceProgramHandle SurfaceProgramStore::Allocate(std::span<const std::uint8_t> artifact) {
    ...
    if (artifact.size() < 27 || artifact.size() > 512 * 1024 || artifact[0] != 'F' ||
        ...
        throw std::invalid_argument("render.surface_program_artifact");
    ...
    if (found == slots_.end()) {
        if (slots_.size() >= 32) throw BudgetExceeded(Budget::kBackendResources);
        slots_.emplace_back();
        found = slots_.end() - 1;
    }
...
bool SurfaceProgramStore::Owns(SurfaceProgramHandle handle) const {
...
void SurfaceProgramStore::Release(SurfaceProgramHandle handle) noexcept {
...
void SurfaceProgramStore::Validate(const SurfaceProgramInput& input) const {
    if (!IsValid(input.program_) || !std::isfinite(input.seconds_) || input.seconds_ < 0 ||
        ...
        throw std::invalid_argument("render.surface_program_input");
}
void SurfaceProgramStore::AddStats(FrameStats& stats) const {
    stats.surface_program_bytes_ = bytes_;
    stats.surface_programs_ = std::uint32_t(std::count_if(
