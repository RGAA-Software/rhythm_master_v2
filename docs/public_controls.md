...
混合是纯数值线性插值，进度范围 [0,1]；控件模型没有第二套时钟。当前提供手动
混合。定时 Cue、自动过渡、跨作品资源准备与双场景合成仍属于 R5 后续任务。

## 保存与平台边界

GraphProject 字段 11、CompiledProgram 字段 7 保存 ControlMetadata；控件范围/
默认值从 control.scalar 节点读取，元数据只存显示名称及快照。
现有 schema 2/3/4 和 runtime ABI 2 的无控件工程继续有效；新增可选字段不改变
旧字段语义。旧运行器不认识 control.scalar，必须以未知操作符拒绝这类新运行包，
不能忽略控件后错误播放。前置 wire 检查限制集合/名称大小，编辑工程保留未知字段。
