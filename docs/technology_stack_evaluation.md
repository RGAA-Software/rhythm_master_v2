...
全向阴影已通过 Windows D3D11 六面、跨面 PCF 像素、实际作品和逐 view GPU 计时，Android 仍待实机。IBL 采用 Godot
MIT 分层 GGX 过滤、粗糙度 LOD、环境 BRDF 和既有 GLM MIT 颜色转换；TiXL 保留同源
Hammersley 序列的历史溯源，没有新增库；见
...
Filmic、ACES、AgX/AllenWP 及 CPU 参数公式。`Vector Resonance`、`Spectral Corolla`、
`Glyph Current` 和 `Phase Plumes` 已从 blur 拼装迁移到独立 glow，并在输出前接入 AgX；
其余作品迁移与合成模式视觉评审继续推进，
