...
| P1 | 阴影 | 已采用 Godot Nearest/PCF5/PCF13、稳定方向光投影、可选双级联和 cube 模式点光全向阴影；越界 PCF tap 会切换相邻面，固定边缘、六方向、跨面软核、子网格、Porcelain Bloom、Chromatic Loom 和 Sonic Enamel 均有 D3D11 回读；仍只有单选择光源 | 继续评审与 Godot 硬件 cube 比较的差异、Android 实机、代表作品和多周期，并补隔离 GPU 成本；四级联/分割混合按实际作品缺口再决定，保留 PCF5 和单图作为兼容默认档 |
