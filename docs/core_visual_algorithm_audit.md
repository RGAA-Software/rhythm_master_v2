...
| 已交付 | 抗锯齿 | 对照 Godot MSAA/SMAA/TAA/FSR1/FSR2 完成路径评估（`provenance/aa_paths.json`）：保留按节点可选 FXAA；TAA/FSR2 的运动矢量依赖与刻意时域效果冲突已拒绝，MSAA 触及不到纹理链锯齿主源，SMAA 记为候选升级，FSR1 留 Android 性能阶段 | Windows 细线/粒子亚像素运动对照（`windows_aa_motion_gpu`）量化：细线阶梯摊平（分数覆盖 0→108）但峰值保留、粒子峰值 255→234.5、时域闪烁均未消除——与审计预期一致，数值断言已固化为永久回归 |
