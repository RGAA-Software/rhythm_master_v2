...
## 2026-09-08 整曲循环的连续输出队列

整曲循环提前准备下一轮 PCM，保持同一个输出流；播放消费到边界后才切换分析和
时间代次。真实 Windows 音频连续两轮计数、源文件保留/替换、暂停/跳转、音频时钟、
大小音乐包及 USB Android 分析边界测试通过。见
[连续循环规格和限制](validation/continuous_music_loops_2026-09-08.md)。没有用交叉淡化
掩盖源波形跳变，也没有将队列连续性表述为硬件无缝或长稳验收。

## 2026-09-08 独立轴缩放与三维音乐场景

三维变换支持独立 X/Y/Z 控制，旧包保留原有等比缩放；Windows 与 Android 的
完整像素边界和共享网格测试通过。律动之城使用 290 个节点、32 个频段驱动
64 根音柱，完成 Studio 保存/发布/恢复、16 秒 H.264 导出和手机音乐/静音对照。
手机原生 640×360 p95 5.11 ms，不代表 APK 验收。见
[场景与验证记录](validation/harmonic_city_2026-09-08.md)。现有 32 个可运行示例，
基础/高级各 50 个的品质验收目标仍未完成。

## 2026-09-08 Windows 三十分钟音乐长稳

208 指令编排作品连续运行 1,800 秒、107,340 帧和 112 次音乐循环，设备消费计数
连续，图形纹理稳定在 113,142,788 字节，结束后释放为零。并行构建/测试负载下
p50/p95 为 16.7/17.2 ms，存在三次约 200 ms 慢帧，不能宣称完全无卡顿。见
[长稳范围与记录](validation/performance_soak_2026-09-08.md)。这是 Windows 验证，
不替代 Android APK 生命周期、热稳定或硬件音频无缝验收。

## 2026-09-08 节点画布每帧开销

移除空闲帧的两次完整工程快照复制，改为实际编辑时才创建事务；同类型节点在
一帧内共享接口描述。千节点拖动/连接/删除、抓手平移、组件和时间编辑回归通过。
