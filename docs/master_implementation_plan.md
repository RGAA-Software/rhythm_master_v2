...
合同、双语言组件库交互和两套 Windows deploy 回归；它不增加候选组件数量。详见
[组件连续运动接入](validation/component_motion_phase_2026-09-10.md)。下一批才提取四件
作品中尚未独立公开的相机、穿行、旋转和粒子控制结构。
...
| 3 | 提取有实际用途的运动组件（P7.2） | 从相机绕行、循环穿行、分层旋转、连续噪声迁移和粒子能量调制中提取可编辑组件；先盘点已有 30 个避免重复 | 每件有输入、公开控件、独立用途、示例与真实动态验证；逐步达到 40 个目标 |
