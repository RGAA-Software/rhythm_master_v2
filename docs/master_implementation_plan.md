...
> 2026-09-11 当前完成度、最新内容口径和后续执行队列见
> [当前完成度与后续实施路径](current_completion_and_execution_plan.md)。该页替代本文
> 第 16 节的 2026-09-10 队列；本文其余 P0–P9 合同和历史证据继续有效。

2026-09-10 Windows 优先：按用户最新指示，本轮仅处理 Windows。
光门单独播放约 60 FPS，低帧率主要在大图 UI 数据翻译与不可读细节生成；
预分配和屏幕细节适配后，Studio 全图中位帧由约 34 ms 降到约 16 ms。
见 [定位、数据和限制](validation/windows_scene_performance_2026-09-10.md)。
这是一项 P8.5 增量，不代表所有性能、P8 或 P9 通过。连续改速相位已完成 Windows
增量：`time.phase` 已接入 Studio、Player 和四件 0.4.0 作品；自然音频循环保留运动和
模拟历史，显式定位/新源仍重置历史。九项 Windows 交付检查、四件实际 Studio 有声
导出和替换音乐后保存/发布/重开检查均已通过，详见
[相位基础时钟与接入契约](validation/motion_phase_2026-09-10.md)。
P7.2 先交付场景充实增量：复用现有 GPU 粒子、2D 物理和三维节点，为高级作品补齐
背景流场、前景火花、受碰撞刚体碎片和持续镜头运动；随后把当前作品中已有的相机绕行、
循环穿行、分层旋转、连续噪声迁移和粒子能量调制提取为独立可编辑组件，再扩展作品批次。
Android、长稳、Apple、通信和透明输出仍后置；不把 Windows 短测当作这些范围已经完成。

P7.2 首个连续性增量已完成：11 个既有自主运动组件改接 `time.phase`，并通过组件
合同、双语言组件库交互和两套 Windows deploy 回归；它不增加候选组件数量。第二个
增量已从当前四件高级作品及敦煌飞带中提取环绕光冕、动势花芯、矿彩流移、无尽光廊
和飞带星图等十个组件，组件候选由 30 增至 40，达到本阶段组件数量目标。详见
[组件连续运动接入](validation/component_motion_phase_2026-09-10.md) 和
[概念作品运动组件提取](validation/motion_component_extraction_2026-09-11.md)。这些组件
通过 Windows 运行包、插入/历史/60 帧执行与 D3D11 缩略图验证；Android 和品质审核
仍未计入。

2026-09-10 动态硬规则：用户认可第二轮外观方向，但要求作品具备持续流动、
旋转、前进和摄像机运动，音乐只作调制。已纳入 AGENTS.md；四件作品按
[第三轮主运动实施与验证](validation/concept_motion_2026-09-10.md) 推进。
P7 后续库存审查也须增加静音主运动、连续周期和回收边界，不能把先前通过的
静态截图或音乐像素差视为已满足新规则。其他库存尚未据此全部复核。

2026-09-10 第二轮视觉修订：用户退回四件 0.1 小样。按概念结构重新组合
GPU 粒子、三维网格、景深、PBR、阴影与倒影能力，详见
[差距复盘与第二轮验证](validation/concept_visual_gap_2026-09-10.md)。
版本修订不增加模板数量，视觉验收继续待用户确认。

2026-09-10 视觉作品更新：四格概念方向均已实现为可编辑的音乐作品，
见 [实际截图、自评与交付验证](validation/concept_works_delivery_2026-09-10.md)。
目录增至 57 模板（19 高级、6 基础、32 示例），组件仍为 30；
这表示内容实现数量，未计作新增用户审美验收，也不表示 P7–P9 已关闭。
...
2026-09-10：2D/3D 两份从零创作示例已接通逐节点添加/配色/命名连接、音乐绑定、
视图平移/旋转/缩放、组件封装和保存发布重开，并通过同一发布包的真实 PCM/GPU
对比。两件内置作品的 Android APK 包内音频/GLES 对照和应用内选择、暂停/恢复
截图核对已完成；详见 [从空画布创作验收](from_empty_authoring.md)。
P4 退出作品功能路径已具备证据，内容视觉品质仍待审核，不计入 P7 已审核品质数量。
详细范围与未完成部分见 [直接编辑记录](view_manipulation.md)。
2026-09-10 P5：文字节点、完整中文字体与许可资产、布局/缺字/裁剪合同、后台准备、
schema 8 / ABI 6 和 Windows 实际空白创作/保存发布/重开及 PCM/GPU 图像已通过。
...
P6.3 已完成相交透明／移动细线的 D3D11 和 GLES 基线及 1080p 短成本测量，
据此限制采用默认关闭的 2× 场景超采样。“共振星仪”0.2.0 已通过实际创作／
...
P6.3 后续透明比较已完成 D3D11／GLES 像素、遮挡及短成本验证：所测单位权重
OIT 虽消除顺序变化，仍丢失前后色彩关系，手机耗时约 2.9 倍、附件量 2.5 倍，
本轮不采用。与已交付可选 2× SSAA 一起形成 P6.3 分项决定并收口；这不表示
相交透明问题已修复，也不推广到未测的其他 OIT 算法。详见
[画质评估](render_quality_evaluation.md)。下一项 P6.4；P7–P9 尚未完成。

P6.4 已完成 1002 节点按需捕获／预览／资源释放两端合同和 D3D11／GLES
几何法线诊断像素探针。保留显式颜色／深度，本轮不新增通用法线／对象 ID
附件；GPU 变形拾取及屏幕空间法线后处理仍未实现。详见
[附加输出评估](scene_output_evaluation.md)。P6 四项均已形成有界决定，
被采用增量已交付；继续 P7 内容品质与 P8 整体工作流，最后执行 P9。

P7.1 已建立源码哈希绑定的独立审核账本及失效／去重检查：当前53模板、29组件、
206预设记录，117条默认值排除品质计数。先复核墨潮、瓷光钟摆、织光机、共振拱廊
两基础＋两高级，未把功能通过自动计为品质合格。P8期间同时修复Android竖屏
时间行裁切，实际横／竖／横、定位、全屏返回和混合节目单短回归通过，见
[裁切修复](validation/android_portrait_transport_clipping_2026-09-09.md)。

P7 首批四作品完成当前 Windows 真实 PCM 对照、16 秒实际 Studio 编排导出，
Android 包内 PCM 与当前 APK 内置选择／暂停恢复。瓷光钟摆曾在 Android
第 2 秒失败；0.2.0 保留原门槛与音乐后通过四时刻对照，详见
[开场响应修复](validation/porcelain_opening_response_2026-09-10.md)。
当前仍需视觉修订、参数极端值／中频／响度及修改交付工作流，未提升品质合格数。
P8 暂停按钮换行基线裁切也已修复，当前 APK 横／竖／横完整控件断言通过；
手机来电中断后的输入工具按当前焦点显示屏拒绝误操作，音频焦点功能仍待验收。

P7 扩展到七组真实 PCM／十项图像对照后发现三个窄带不能代表完整低中高频。
瓷光钟摆 0.3.0 已复用现有节点覆盖全部 63 频带，并修正地面硬边取景，
211 节点版本的 Windows 模板应用／导出与 Android 当前包音乐／实际 UI 已通过。
见 [频段覆盖修复](validation/porcelain_frequency_coverage_2026-09-10.md)。
墨潮和织光机也暴露中频不足，进入同类修订；参数极端值和完整品质验收仍继续。

首批其余三件的宽频段修订也已交付：墨潮 125、织光机 297、共振拱廊 247
指令，全部通过七 PCM／十项 D3D 对照、当前 APK 四时刻真实音乐与内置 UI，
以及实际 Studio 模板应用／有声导出。详见
[校准批次证据](validation/calibration_band_coverage_2026-09-10.md)。
审核账本当前四件的 music 阶段通过，其余阶段仍 pending；不增加品质合格数。
导出期间复现并修复 Windows 进度文件与原子发布的共享冲突，确定性合同、
完成／取消／子进程失败及实际 480 帧导出已复核，见
[进度共享修复](validation/export_progress_sharing_2026-09-10.md)。
四件作品现已补齐公开控件与配乐修改交付：12 个控件共 24 次实际鼠标上下限
操作，当前编译／保存／重开／发布通过；固定真实 PCM 下全部 12 项控件图像
对照通过。更换整份配乐也通过内容哈希、清除和重开检查，并修复旧测试只判断
“存在配乐”会误认旧音乐的验证漏洞。详见
[公开控件与修改交付](validation/calibration_control_delivery_2026-09-10.md)。
审核账本四件 functional／music 通过，visual／device 仍待最终审核，品质合格数不增加。
下一步是最终视觉修订、当前源对应的手机声明档位，再扩展组件及作品批次；
...
织光机 0.3.0 完成斜向构图、细线交错和双光梭短轨迹修订，复用已有网格／材质，
保留三频段与三宏／四 Cue。当前源的七份真实 PCM、控件上下限、实际模板应用／
换配乐／保存发布、16 秒有声导出、Android APK 内音乐渲染与应用选择暂停恢复通过，
见[织光机修订证据](validation/chromatic_loom_depth_2026-09-10.md)。
属于同一作品修订，不增加数量；连续运动听感与最终视觉品质仍待审核。

P7.2 新增“三频折屏”输入组件，图像／文字等纹理可连接到三块音乐驱动的
立体屏幕；复用现有贴图材质、共享网格和完整三频段，六项公开参数与紧凑预设。
30 组件合同、实际中文组件库操作、七组真实 PCM 和 Android 原生 GLES 短测通过，
见[三频折屏组件证据](validation/spectral_triptych_component_2026-09-10.md)。
组件候选现为 30 / 40，预设记录 208 条；仍须审核独立性和最终品质，
不将默认参数或组件预览图计为已验收成品。
...
- 当前提交中的 `content/templates/*/manifest.json` 共 **21** 个，全部为 Advanced；简单和
  实验模板已按当前内容方向移出目录。21 个均为 `visual-review-pending`，不能当作用户验收数量。
- 语义组件 **40** 个；提交中的预设记录共 **235**，其中 132 条默认值记录被品质计数排除。
- 目标仍是 **40 个语义组件、120 个视觉独立合格预设、50 个高级合格作品**。
  当前没有建立正式品质验收总数；不能把 21 个候选或 235 条参数记录当作合格数量。
  达到候选数量和达到品质目标分别记账。
...
视觉制作采用[视觉设计先行流程](visual_design_workflow.md)：可参考成熟作品或先
生成多风格概念图，再拆成可编辑、音乐驱动的实时图。第一张四风格方向板已保存，
它属于设计目标，不是实现或验收证据；正式缩略图仍由实际 GPU 输出生成。
...
## 16. 当前执行队列（2026-09-10，基线 0115f3e）

四件 0.3 主运动、相机轨迹、静音／音乐导出与 Windows 交付已完成并推送。
用户要求先看 Windows 效果，Android 画面检查暂缓；这覆盖上文历史滚动状态中
“先完成手机声明档位”的顺序。以下是当前下一步，后面的初始队列仅保留历史。

| 顺序 | 工作 | 实施路径 | 本批退出条件 |
| --- | --- | --- | --- |
| 1 | 光门实时性能（P8.5） | 固定当前 703 指令作品、音乐、分辨率与预览状态；分测 Studio 开／关节点预览及 Player，定位 CPU 图执行、场景整理、提交和可用 GPU 数据；按实测瓶颈修复 | 保留优化前后同条件 p50/p95、峰值与资源数据；真实画面／音乐／保存发布回归通过，不以降低视觉质量或离线导出帧率替代优化 |
| 2 | 连续改速的运动相位 | 已完成 0.4.0 Windows 增量；保留时间/循环/显式定位、GPU 历史和真实导出证据 | 九项交付检查、四件有声导出、替换音乐保存发布重开均通过；Android/长稳未计入 |
| 3 | 提取有实际用途的运动组件（P7.2） | 已提取十个组件，候选数 30→40；粒子能量、点阵舞台、门架视差、折叠轨道和螺旋灯标补齐，后续只按作品实际缺口扩展 | 每件有输入、公开控件、独立用途、示例与真实动态验证；40 个目标已达到 |
| 4 | 新运动规则复核与作品批次（P7.1／P7.3／P7.4） | 原四件校准作品持续复核；P7.3 首个候选“星铸圣坛”已完成 Windows 功能、三频段、六控件与真实 Studio 应用检查；第二件“琉璃天门”已完成 32 秒周期闭合动态复核、七段 PCM、六控件极值、真实 Studio 应用/有声与静音导出/换曲检查。继续按概念／动态草案→实现→音乐和静音视频→自评推进独立作品 | 当前库存为 22 个模板、40 个组件；星铸圣坛与琉璃天门仍为 visual-review-pending。不得换色或复制图凑数量，最终目标仍是基础／高级各 50 件合格作品 |
| 5 | Windows 完整创作与演出闭环（P8） | 按现有从零创作、修改模板、组件复用、连续切场和导出实际入口补齐缺项；发现错误就修复关联路径 | 当前源码对应的 Studio／Player 输出与交付证据齐全；不重复用已通过的孤立测试代替完整操作 |
| 6 | 恢复 Android 画面验收，最后 P9 | 用户当前暂缓手机画面；恢复时先用当前 APK 核对内置选择、真实音乐、控件与生命周期，之后才执行最终长稳／热测试与发行材料 | 未测项保持待验收；仅覆盖安装并保留数据。Apple、通信和透明输出继续后置 |

第一项已有问题基线：光门本轮 Studio 导出期间父窗口 p50 41.57 ms、
p95 50.42 ms；它是定位起点，尚不是严格可比的优化基准，更不是 GPU 时间。
详见 [本轮证据与限制](validation/concept_motion_2026-09-10.md)。
现有 57 模板（6 基础、19 高级、32 示例）、30 组件、208 个非默认预设记录
是作者库存，不能当作已完成品质／平台验收的数量。

### 初始提交队列（历史，已被上述当前队列替代）
...
初始计划按 P2→P3→P4→P5 推进，每批穿插 P7。此历史队列当时替代旧文档中已经过时的
