...
相交半透明问题仍待独立方案比较；2026-09-11 已增加可选 alpha depth prepass，按
Godot 的 0.99 覆盖阈值只让接近不透明的区域提前写深度，并保留透明纹理孔洞。它改善
树叶、网格等材质与后层的遮挡，不是通用 OIT。OIT、TAA 和 MSAA 仍未采用；2× 超采样
不会修复排序，不能以本增量宣布全部 P6.3 完成。
...
## 相交透明比较与决定（2026-09-10）

本地 bgfx example 19 的累积／归一化／透射率路径已检查，并固定到 vcpkg
缓存源码包 v1.129.8940-496，BSD-2-Clause 来源见 `provenance/bgfx_oit.json`。
此次先取单位权重的近似分支：RGBA16F 累积预乘颜色和 alpha，第二次几何绘制
累乘透射率，最后归一化并覆盖到不透明背景。使用两次几何 pass，避免先假设
Android 支持 MRT 独立混合，也能探索复用既有材质片元输出的可能。

原生探针对 alpha 0.1／0.5／0.95 的相交红蓝平面交换顺序，分别记录顺序敏感性、
相对正确逐像素前后顺序的颜色误差，以及前景不透明绿色条的深度遮挡。
另以 1080p 八层、12 帧测附件请求量和提交＋32×16 小块回读成本。
两端探针均通过，绿色前景深度遮挡保留，单位权重分支交换绘制顺序的像素差为零。
但它把应有前后色彩主导关系变为平均混合，不能称为精确透明排序。

| 后端／alpha | 排序基线平均误差 | 单位权重平均误差 | 基线最大顺序差 | 单位权重最大顺序差 |
| --- | ---: | ---: | ---: | ---: |
| D3D11 / 0.1 | 1.275 | 1.275 | 2 | 0 |
| D3D11 / 0.5 | 31.875 | 31.875 | 63 | 0 |
| D3D11 / 0.95 | 115.125 | 115.069 | 230 | 0 |
| GLES / 0.1 | 1.525 | 1.275 | 3 | 0 |
| GLES / 0.5 | 32.125 | 31.875 | 64 | 0 |
| GLES / 0.95 | 115.125 | 115.069 | 230 | 0 |

误差单位为 8 位通道值，比较 x=8/24 的红蓝通道与逐像素正确排序的解析结果，
不是整幅图像 MAE。两种提交顺序的平均误差相同。alpha=0.95 时，单位权重最大
通道误差仍约 115，而正确图像应在左右分别由红色／蓝色主导。
人工查看 `out/p6-transparency-d3d/comparison-95.png`：依次是解析参考、两种
排序基线、单位权重近似；最后一幅确实两侧均为紫色，前景绿色条保留。

| 后端 | 基线每帧提交＋回读 | 单位权重每帧提交＋回读 |
| --- | ---: | ---: |
| D3D11 | 9.941 ms | 16.0889 ms |
| Redmi K40S / Adreno 650 GLES | 47.2733 ms | 135.428 ms |

1080p 八层无光照几何、12 帧；这不是纯 GPU 时间、稳定 FPS 或完整 PBR 测量。
请求附件量从 16,588,800 增至 41,472,000 字节（2.5 倍），不含几何、程序、
驱动开销和 2048 字节 staging。原生探针没有经过产品资源表的预算准入。
不能与前面的整图回读 SSAA 测量直接横比。

**决定：本轮不采用这个单位权重 OIT 候选。** 它的顺序稳定性收益没有修复主要
遮挡／颜色缺陷，手机探针成本约为基线 2.9 倍，不满足本轮作品收益和预算门槛。
只保留可复现的原生验证工具与来源，不增加 Runtime／材质模式。
此结论不等于否定所有深度权重 OIT、深度剥离或其他透明算法；那些没有在本次
验证，也没有获得产品支持。通用相交透明依然是已知限制。

P6.3 以“限制采用可选 2× SSAA；不采用所测单位权重透明候选”的分项决定收口，
不是透明画质问题全部解决。2× SSAA 的正式交付证据见上文。

验证日志：`out/p6-transparency-profile-build.log`、
`out/p6-transparency-profile-d3d.log`、
`out/p6-transparency-profile-android-build.log`、
`out/p6-transparency-profile-gles.log`。
全部数字与 12 幅原始 PPM 分别在 `out/p6-transparency-d3d/` 和
`out/p6-transparency-gles/` 的 `measurements.txt` 及同级文件中。
Android 执行的是 USB 原生探针，不冒充 APK 应用路径验收。
